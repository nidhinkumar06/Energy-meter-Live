...
const sampleConfig = `
  ...
  ## Total Slaves - total number of slaves - has to be the same for all instances of modbus config.
...
func (m *Modbus) Gather(acc telegraf.Accumulator) error {
	...
	if current_slot != uint32(m.SlaveID) {
		fmt.Println("Modbus plugin:Skipping Slot : Highest slaveID->", m.HighestSlaveIDinLink, " Slave ID->", m.SlaveID, " slotCounter->", m.slotCounter, " current slot->", current_slot)
		return nil
	} else {
		fmt.Println("Modbus plugin:......Sampling Slot : Highest slaveID->", m.HighestSlaveIDinLink, " Slave ID->", m.SlaveID, " slotCounter->", m.slotCounter, " current slot->", current_slot)
	}
